...
def handle_Contract(r_string, target_gdb):

    r_dir = (os.path.join(os.path.abspath(os.path.join(target_gdb, "../..")), 'DELIVERED', r_string[-3:]))
...
def get_token(username, password, base_url):
    ...
    try:
        ...
        if r.status_code != 200:
            logger.info('Server Did Not Return 200 When Requesting Token: {}'.format(r.status_code))
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Collect Input DB & Verify Existence
        gdb = os.path.join(base_dir, arcpy.GetParameterAsText(0))
        dsc = arcpy.Describe(gdb)
        if dsc.extension != 'gdb' and dsc.dataType != 'Workspace' and arcpy.Exists(gdb):
            raise Exception('Input File Geodatabase Could Not Be Loaded')
        
# #        # Testing Geodatabase
# #        gdb = r'Q:\LiDAR\OK_SugarCreek_2008\DERIVED\OK_SugarCreek_2008.gdb'

        # Collect Token For REST Insertions
        token = get_token(username, password, base_url)

        # Collect Project ID as DB Name
        project_id = os.path.split(gdb)[1].split('.')[0]
            ...
            for cmdr_target, mapping_dict in features.items():
                ...
                for db_ob, mapping in mapping_dict.items():
                    ...
                    if db_ob not in [
                        ...
                        feat_target = os.path.join(gdb, db_ob)
                        ...
                        rast_target = os.path.join(gdb, db_ob)
                ...
                # Insert Collected Values Into Feature Service
