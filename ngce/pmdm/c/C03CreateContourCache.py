...
def CreateContourCache(jobID, serverConnectionFile):
    ...
    Utility.printArguments(["WMX Job ID", "serverConnectionFile", "cache_dir"],
                           [jobID, serverConnectionFile, cache_dir], "C03 CreateContourCache")
...
#         if folder is not None and len(folder) > 0:
#             cacheFolder = os.path.join(cacheFolder, folder)
        ...
        #-------------------------------------------------------------------------------
        #-------------------------------------------------------------------------------
        Utility.printArguments(["mxd", "areaOfInterest", "serviceName", "folder", "sddraft", "sd", "tilingScheme", "cacheFolder"],
                              [mxd, areaOfInterest, serviceName, folder, sddraft, sd, tilingScheme, cacheFolder], "C03 CreateContourCache")
        ...
        arcpy.mapping.CreateMapSDDraft(mapDoc, sddraft, serviceName, "ARCGIS_SERVER", localServer, folder_name=folder)
        ...
        arcpy.CreateMapServerCache_server(inputService, cacheFolder, "PREDEFINED", predefined_tiling_scheme=tilingScheme, scales=scales)  # , scales_type="STANDARD", num_of_scales=len(scales))
        ...
        arcpy.ManageMapServerCacheTiles_server(inputService, scales, updateMode, cachingInstances, areaOfInterest, updateExtents, waitForJobCompletion)
        ts = time.time()
        st = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
